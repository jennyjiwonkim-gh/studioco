...
- [ ] Decide: fix `savePersisted` silently swallowing failed saves
      (`index.html` ~556-569, currently `console.warn` only, no UI
      indication)? Raised when the photo data-loss bug was investigated;
      never implemented. Originally surfaced via an ambiguous Jenny question
      that was never fully resolved — confirm this is actually what she
